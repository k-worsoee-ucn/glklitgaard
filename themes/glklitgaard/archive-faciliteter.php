...
<section class="bg-gray-200 grid gap-6 relative grid-cols-1 w-full h-fit wavey-section z-10">
    ...
    <?php while($Faciliteter->have_posts()){$Faciliteter->the_post(); if($BelejAmnt % 2 != 0){?>
            ...
            <div class="w-full mx-auto grid grid-cols-1  bg-yellow-100 py-8 pl-10">
                ...
                <?php if(has_excerpt()){echo get_the_excerpt();}else{
                   ...
                   if(preg_match($pattern, $content)){
                    ...
                    } else{echo wp_trim_words(get_the_content(), 100, "..." );}
                } ?></div>
                <a href="<?php echo get_the_permalink(); //ligende med tidligere ting, men vi henter linket fra en seperat fil og sætter ind som href'en.?>" class="knap text-center px-3 py-1 mx-auto w-fit mt-5 font-semibold grid">Læs mere</a>
            </div>
...
<?php }; $Faciliteter = new WP_Query(array(
            ...
            'meta_value'    => 'Sjov',
            'order' => 'ASC',
        ));if($Faciliteter->have_posts()){ $SjovjAmnt = 1; $flipFlop = 1;?>
